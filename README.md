...
完整代码 [sudoku-player](https://github.com/psychopurp/sudoku-player/main.cpp)
...
终端输入 `./main -n 1 -u` 会生成 1 个解唯一的数独游戏到 game.txt 的文件。
game.txt
...
终端输入 `./main -s game.txt` 会读取 game.txt 文件里生成的数独游戏，并将求解后的答案输出到 sudoku.txt 文件中。
sudoku.txt
